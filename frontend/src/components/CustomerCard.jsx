...
export default function CustomerCard({ customer, isSelected, onCardClick }) {
  const statusStyles = getStatusClasses(customer.status)
  const churnStyles = getChurnRiskClasses(customer.churnScore)

  const className = clsx(
    'card block hover:shadow-2xl hover:-translate-y-2 transition-all duration-700 ease-out group',
